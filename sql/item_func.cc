...
longlong Item_func_dbmssql_parse::val_int()
{
  ...
  String *stmt_cmd=args[1]->val_str(&stmt_cmd_value);

  char new_stmt_name[12]= "";
  sprintf(new_stmt_name, "s%i", cursor_id);

  for (size_t i = 0; i < thd->cursor_list.elements(); i++)
  {
    if (thd->cursor_list[i].cursor_id == cursor_id)
    {
      char *stmt_cmd_cp_ptr= (char *) malloc(stmt_cmd->length());
      strcpy(stmt_cmd_cp_ptr, stmt_cmd->ptr());
      thd->cursor_list[i].cursor_statement.set(stmt_cmd_cp_ptr,
          stmt_cmd->length(), stmt_cmd->charset());
      break;
    }
  }
  return 0;
