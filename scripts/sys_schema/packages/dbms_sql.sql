    ...
    BEGIN
      ...
      BEGIN
        ...
        ELSIF first_word IN ('insert', 'update', 'delete') THEN
          SET final_ps = CONCAT('CREATE PROCEDURE final_NESTED_PROC() AS BEGIN PREPARE s', cursor_id, ' FROM \'', input, '\'; END');
        END IF;
        EXECUTE IMMEDIATE final_ps;
        EXECUTE IMMEDIATE 'CALL final_NESTED_PROC()';
        EXECUTE IMMEDIATE 'DROP PROCEDURE final_NESTED_PROC';
        SET parse_result = dbms_sql_parse(cursor_id, first_word);
      END;
